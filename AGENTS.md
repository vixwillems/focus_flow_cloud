...
- The widget's `IPHONEOS_DEPLOYMENT_TARGET` is **17.0** (needed for `containerBackground(for: .widget)`). The main app is at 16.0 (needed for `Text(timerInterval:)` to drive the Live Activity / widget countdown text off the iOS wall clock).
